...
标记点不只是播放书签，也是用户主动表达学习意图的分析锚点。用户可以为标记设置 0～5 星重要程度，系统按星级提高附近内容的分析权重，并结合语音、画面和上下文生成更贴合个人关注点的笔记。
...
- 页面刷新后恢复媒体列表、手工时间点或范围标记及其重要程度；标记保存在对应媒体资产目录中
- 使用原生 HTML5 播放器播放，支持 HTTP 单区间 Range 和进度条拖动
- 视频就绪后在后台渐进生成转录、章节、关键帧、OCR 与检索索引；界面显示真实阶段和已覆盖范围
- 优先复用平台字幕，缺失时使用本地模型提取带时间戳文字；本地神经嵌入与重排模型按需下载并支持索引代际原子切换
- 支持全片时间轴与标记重点两种分析模式，按语音停顿和画面变化组织课程事件
- 为每个事件提取多张时序画面；视觉模型不可用时仍保留可回跳的音频分析结果
- 使用一个全局侧边助手完成视频问答、总结编辑和转录修正，显式附件、检索范围与会话上下文随工作对象切换
- 结合 SQLite FTS5、语义检索、已验证记忆和神经重排生成证据引用；答案使用高、中、低三档置信度，时间证据可驱动播放器定位
- 通过 LiteLLM 注册在线 API，并按自动、快速或复杂思考模式选择快速文本、复杂文本或唯一视觉模型
- 前台对话优先于后台模型任务；瞬时限流使用有限退避重试，流式事件通过可恢复 SSE 推送并以持久化序号续传
...
Vite 会把 `/api` 代理到 `http://127.0.0.1:38471`。
...
1. 为互联网知识检索单独评审来源可信度、引用和权限边界
2. 评审满意度、撤销与重试等反馈是否用于自动修正策略
3. 持久化和恢复服务重启前未完成的下载任务
4. 支持媒体库搜索、标签筛选与批量管理
5. 在复用现有 API 与媒体库的前提下，增加 Qt 原生工作台
