...
SUMMARY_ARTIFACTS_DIRECTORY = "summaries"
...
class SummaryManager:
    ...
    def generate(
        ...
        existing = self.library.load_summary_documents(asset_id)
        if existing:
            raise SummaryError("该视频已经生成总结文档")
        model = self.settings.ai_model(request.ai_model_id) if request.ai_model_id else None
        ...
        self.library.create_summary_document(root)
        for child in children:
            self.library.create_summary_document(child)
    ...
    def create_child(
        ...
        children = [
            document
            for document in self.library.load_summary_documents(root.asset_id)
        ...
        return self.library.create_summary_document(document)
    ...
    def reorder_children(self, root_document_id: str, document_ids: list[str]) -> list[SummaryDocument]:
        self.library.reorder_summary_documents(root_document_id, document_ids)
        root = self._require_document(root_document_id)
        return self.library.load_summary_documents(root.asset_id)
    ...
    def conversation_state(self, asset_id: str) -> SummaryConversationState:
        ...
        root = next((document for document in documents if document.parent_document_id is None), None)
        if root is None:
    ...
    async def _execute_agent_run(
        ...
        running = run.model_copy(
            update={"stage": SummaryAgentRunStage.RUNNING, "updated_at": datetime.now(UTC)}
        )
        ...
        if model is None:
            ...
            return
        documents = self.library.load_summary_documents(document.asset_id)
    ...
    def accept_proposal(self, proposal_id: str) -> SummaryEditProposal:
        ...
        for suggestion in proposal.suggested_subdocuments:
            root = next(
                (
                    item
                    for item in self.library.load_summary_documents(document.asset_id)
        ...
        accepted = proposal.model_copy(update={"status": SummaryProposalStatus.ACCEPTED})
    ...
    def reject_proposal(self, proposal_id: str) -> SummaryEditProposal:
        ...
        if proposal.status != SummaryProposalStatus.PENDING:
            return proposal
        rejected = proposal.model_copy(update={"status": SummaryProposalStatus.REJECTED})
        self.library.save_summary_proposal(rejected)
        return rejected

    async def create_media(self, request: SummaryMediaCreate) -> tuple[SummaryMediaArtifact, SummaryDocument]:
        document = self._require_document(request.document_id)
        if document.revision != request.expected_revision:
        ...
        relative_path = f"artifacts/{SUMMARY_ARTIFACTS_DIRECTORY}/{media_id}{suffix}"
        output_path = self.library.asset_directory(asset.asset_id) / relative_path
        try:
        ...
        markdown_path = f"assets/{media_id}{suffix}"
    ...
    def export(self, asset_id: str) -> tuple[str, bytes]:
        asset = self._require_asset(asset_id)
        documents = self.library.load_summary_documents(asset_id)
        root = next((document for document in documents if document.parent_document_id is None), None)
        if root is None:
            raise SummaryNotFoundError("请先生成主文档")
        children = [document for document in documents if document.parent_document_id is not None]
        ...
        with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            for child in children:
                child_markdown = child.markdown.replace("](assets/", "](../assets/")
                archive.writestr(f"docs/{child.document_id}.md", child_markdown)
            for artifact in media:
                source_path = self.library.asset_directory(asset_id) / artifact.relative_path
                if not source_path.is_file():
                    raise SummaryError(f"总结资源缺失：{artifact.media_id}")
                archive.write(source_path, f"assets/{source_path.name}")
            archive.writestr(
                "manifest.json",
                json.dumps(manifest, ensure_ascii=False, indent=2),
            )
        return f"{root.document_id}.zip", buffer.getvalue()
    ...
    def _root_markdown(
        ...
        for segment in segments:
            ...
            summary = segment.detailed_summary or segment.transcript_text or "该片段暂无文字说明。"
            if detail == "concise":
                summary = summary[:180]
...
def _parse_agent_response(
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("proposed_markdown"), str):
        return normalized, fallback_explanation, [], []
...
def _insert_markdown(markdown: str, insert_after: str | None, media_markdown: str) -> str:
