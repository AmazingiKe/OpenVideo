...
export function use_storyboard_preview(asset: MediaAsset | null) {
  const query_client = useQueryClient();
  const generating_asset_id_ref = useRef<string | null>(null);
  const request_storyboard = useCallback(() => {
    if (
      !asset ||
  ...
  useEffect(() => request_storyboard(), [request_storyboard]);
